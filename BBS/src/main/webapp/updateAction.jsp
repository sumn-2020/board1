	...
	<%
		...
		if(userID == null) { //할당된 세션이 있을 경우 
		 ...
		 if(!userID.equals(bbs.getUserID())) { //이 글을 작성한 사람의 id가 넘어온 id값이 같지 않다면 
			 ...
			 //update 페이지에서 name값으로 넘어오는 값들 받아서 처리 
			 //request.getParameter("bbsTitle")으로 쓰는 이유: 상단에 beans 없앴기 때문에 beans 없으면  bbs.getBbsTitle() 이런 방식 말고 request.getParam형식으로 값 받을수 있음 
			if(request.getParameter("bbsTitle") == null || request.getParameter("bbsContent") == null 
				|| request.getParameter("bbsTitle").equals("") || request.getParameter("bbsContent").equals("")) { //한 항목이라도 비어있을 경우 
					PrintWriter script = response.getWriter();
					script.println("<script>");
					...
					//write.jsp에서 getBbsTitle과 getBbsContent값을 받아서 dao속에 있는 update메소드  실행
					int result = bbsDAO.update(bbsID, request.getParameter("bbsTitle"), request.getParameter("bbsContent") ); //상단에서 받아온 각각의 name값들을 받은 것들을 매개변수로 사용
